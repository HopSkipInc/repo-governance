...
| **`scripts/check-classifier-pin-drift.mjs`** | agent-routing is applied | P1 | The classifier's `model:` pin must agree with the class map — §3's "every pin resolves to a frontier model" was a human re-sync step, and a human step is the check that stops happening. Resolves the pin slug through the records §2 route table and asserts §1 lists it under the declared `# routing-class:`; fails closed on a missing marker, an unlisted class, or drift. Wire it where the existing repo lints run (agent-routing.md §*One exception*, policy ≥ 1.15.0) | full |
...
| **`audit-domains.md`** | Always — the repo runs an audit at all, by any substrate | P0 | The single definition of what an audit checks: nine domains, severity scale, P2 aging rule, finding-ID tokens. Installed as `docs/audit-domains.md`; every consumer reads it at run time rather than holding a copy. **Applies to fleet/cron-machine audits as much as workflow ones** — in a machine-driven estate the domain list otherwise lives inline in each machine's stored goal, invisible to every file-reading check, and the reference estate's three audit machines had already drifted to different domain sets. Converting a machine costs one migration that swaps the inline block for a pointer; after that, adding a domain is a file edit. `workflows/scheduled-audit.yml` ≥2.0.0 fails its job when this file is absent | full |
| **`configuration-governance.md`** | Repo reads configuration at runtime (env vars, config files, a secret store, or deployment-set settings) | P1 | Two-axis model — sensitivity decides protection, cardinality decides placement; they are independent, and collapsing them is what produces an unauditable `.env`. Carries the reference rule (an env var may hold a secret *reference*, never a secret *value*) and a five-tier credential ladder with a stricter floor for non-human principals, since every cheap human control depends on interactive consent that an agent worker does not have. Enforcement is coverage, not correctness — the paired lint and records file ship separately | full |
...
| **`epic-handoff.md`** | Backlog has epics worked across sessions | P1 | The rolling continuation section — a dated `## ▶ Pick up here` in the epic body, so a restart, a failed worker, parallel sessions, or a night's sleep never costs the next session the epic's live state. Read at session start, refreshed on every child close; the mandatory `Handoff updated` marker is what the probe ages. Pairs with the DoD epic-handoff rows and the single-writer rule for concurrent workers | full |
| **`scripts/check-epic-handoff.mjs`** | `epic-handoff.md` is applied | P1 | Body-only freshness probe — section presence, dated marker, age. It does **not** enumerate children: the body-versus-graph axis is `check-backlog-currency.mjs`'s `TRIGGER children-changed-since-body-update` where the grooming regime runs, and one axis gets one enumerator (PDR-008). Probe, never a gate (ADR-026); `missing` is report-only on first adoption | full |
| **`workflows/epic-handoff-probe.yml`** | `scripts/check-epic-handoff.mjs` is applied | P1 | The weekly cron that runs the probe and upserts one rolling issue. Uses the default `github.token` — no cross-repo scope, because the probe reads epic bodies only. No P1 escalation in v1; promotion and escalation are earned by a false-positive review, never assumed | full |
| **`merge-path.md`** | Pull requests are opened by agents and merged by a human | P1 | GitHub refuses APPROVE from a pull request's own author, so a repo whose agents push under BOTH a bot identity and the operator's own cannot satisfy one approval rule with both — the requirement becomes a bypass habit that configuration alone cannot see. Moves the machine verdict to a check run (no author restriction), keeps the human merge click, and makes it durable via auto-merge | full |
| **`scripts/check-merge-path.mjs`** | `merge-path.md` is applied | P1 | Reads live config AND recent merge history — config alone cannot tell a rule that holds from one walked through weekly. `ungated` (a PR required, no check required) is the only blocking class; the two history classes are threshold-based and ship report-only until calibrated against a real repo, same promotion discipline as `lint-stub-tests.mjs` | full |
| **`workflows/merge-path-probe.yml`** | `scripts/check-merge-path.mjs` is applied | P1 | The only sanctioned wiring: a weekly scheduled probe upserting one rolling issue. Never a per-PR gate — the merge path is not a function of the diff, and a check that could block merges over merge configuration would be its own punchline | full |
...
pass.
The three **epic-handoff** rows were added 2026-09-26 with the rolling-handoff
