...
| **`audit-domains.md`** | `workflows/scheduled-audit.yml` is applied, **or** the repo runs an audit in-platform | P0 | The single definition of what an audit checks — nine domains, severity scale, P2 aging rule, finding-ID tokens. Installed as `docs/audit-domains.md`; consumers read it at run time rather than copying it. Extracted 2026-09-16 because the domains lived inside the workflow prompt and were therefore unreachable by any repo that moved its audit in-platform — one such repo was already running six of the eight domains with nothing reconciling the sets. **Required companion to scheduled-audit.yml ≥2.0.0**, which fails the job when it is absent | full |
