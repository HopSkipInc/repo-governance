...
**DEFERRED FIX (raised 2026-07-24, not done):** `routing-triage/SKILL.md` says "must be run by a frontier-class model" and gates nothing — SKILL.md frontmatter has no harness-honored `model:` field, so it's an honor-system sentence interpreted by the thing it constrains. This directly contradicts the policy the skill ships with, which states the dispatcher is the only real enforcement. **Fix:** split into a thin skill (discovery, interview, `gh` writes) plus `.claude/agents/routing-classifier.md` carrying `model:` frontmatter — agent definitions *are* harness-resolved at spawn, so the classifier never gets a vote on its own model. Unverified second belt: whether the `PreToolUse` hook payload carries a model ID (ai-fleet already has that block wired). Note: self-identification is not the hard part — the harness tells a model its own ID; compliance is.
