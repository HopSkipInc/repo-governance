...
| HopSkipInc/ai-fleet | [2026-08-21 backlog grooming regime](ai-fleet/2026-08-21-backlog-grooming-regime.md) | pending — requests two new templates (`check-backlog-currency.mjs`, `backlog-groom` skill) and carries the v1 design (detection semantics, roadmap schema, §8 extraction gate); step 0 installs the already-shipped `check-stale-blockers.mjs` v1.0.0 rather than rebuilding the blocked-by half. ai-fleet is pilot and reporting repo. Self-test must flag the 2026-08-20 incident's own issues. Estimation column is contract-only per PDR-010 and stays deferred until telemetry coverage moves off ~15% (agent-routing-records.md, 2026-08-17). Trust model: the backlog is the substrate (conditionally — checks green + grooming on the repo owner's cadence), the roadmap is a projection of it plus product-owner statements, estimates layer on the projection |
