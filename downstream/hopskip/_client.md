...
| HopSkipInc/ai-fleet | [2026-08-28 migration numbering concurrency](ai-fleet/2026-08-28-migration-numbering-concurrency.md) | pending — **renames nothing**: all 385 files are journaled, and the `0430` incident (2026-08-20, two PRs nine minutes apart) proved Rule 4's rename fix and Rule 6's immutability are mutually unsatisfiable once both files are journaled. Relaxes Check 4 to key on `NNNN_ISSUE`, requires the token on added files only, amends ADR-008 Rule 4 via `write-record.mjs`, declares the template at v1.1.0 with the flat-prefix deviation. **Separate PR owed:** `.githooks/pre-commit` greps `^db/migrations/host/` and queries `schema_migrations` — both retired by the 2026-07-06 DbUp cutover, so the staged-migration gate ADR-008 lists as a Gate has been inert ~7 weeks |
| HopSkipInc/analytics-infrastructure | [2026-08-28 migration numbering concurrency](analytics-infrastructure/2026-08-28-migration-numbering-concurrency.md) | pending — **the incident has not happened here yet, which is the reason to apply now.** `GRANDFATHERED_DUPLICATE_PREFIXES` is an empty set, `schema.yml` journals on push to main, and immutability is enforced — so the next same-prefix race goes red on `main` with no legal exit but editing the lint under pressure. The cheap-fix window closed 2026-07-05: the prefix burn-down (031→066, 042→067, 043→068, 312→325) finished 2026-07-04, one day before DbUp adoption. Rule 7 (unique base filenames, permanent register) is explicitly **not** weakened — it is being promoted upstream on this repo's evidence |
| HopSkipInc/enrichment-pipeline | [2026-08-28 migration numbering concurrency](enrichment-pipeline/2026-08-28-migration-numbering-concurrency.md) | pending — the template's reference implementation and the only install of its `YYYYMMDD_NNNN` naming, and the census found the collision already living in `NOTES.md` under "Known duplicate sequence numbers (do not rename)". Corrects that file's false claim that "the date prefix provides uniqueness and ordering" (it provides ordering — 11 dates carry 2+ migrations; the registered `20260110_0036` pair orders only by `q` < `t`), drops the `20260102`/`20260103` row that is not a collision, adds the issue token, and fixes `GenerateMigrationCommand.cs` first since this repo owns the template's CLI contract. **No numbering lint** — recorded as a deliberate absence (4 active files), not an oversight |
