...
| HopSkipInc/ai-fleet | [2026-09-20 roadmap layer 3 v2](ai-fleet/2026-09-20-roadmap-layer3-v2.md) | pending — builds the grooming regime's Layer 3 as revised in place (v2, banner at the 2026-08-21 prompt's §3): deterministic data pass emitting a committed, diffable `docs/roadmap-data.md` with a **generated trust header** (probe last-green, groom cadence state, census, >90d debt count; no wall-clock — byte-identical re-runs or every diff is spurious), **two tables** — roadmap plus an age-bucketed **debt table** for the unparented population (412/525 issues, 78%, census 2026-09-20) with `status:deferred` separated and dated — a narrative skill writing marked sections of `docs/roadmap.md` with `file:line@ref` provenance, a **debt-sweep skill** (triage, reproduce-or-close, parenting proposals — graph writes proposed never applied — three-way fix/defer-with-date/close disposition, batched oldest-first), probe-class publishing (weekly PR + on-demand, never a gate per ADR-026 / the 2026-08-18 erratum). Shares the Layer 1 currency lint's graph loader via `scripts/lib/backlog-graph.mjs`; the byte-identical stale-blockers template keeps its own loader — unifying it is an upstream template revision owed at extraction. **No `templates/` artifact until one full cycle runs** (PDR-010 Consequences 4); Layers 1–2's §8 gate is separate and unmet |
