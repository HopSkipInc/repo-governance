...
| HopSkipInc/analytics-infrastructure | [2026-08-21 stale-blockers install](2026-08-21-stale-blockers-install.md) | pending — byte-identical script + probe workflow (both shipped 2026-08-18; no new template work, no pilot gate). Convention check 2026-08-21: 58/60 recent open issues carry `blocked-by`/`## Dependencies`, so expect real first-run findings. The one judgment call is the `STALE_BLOCKERS_TOKEN` estate secret. ai-fleet takes the same install as step 0 of the grooming regime — tracked there, not duplicated here |
| HopSkipInc/ai-fleet | [2026-08-21 backlog grooming regime](ai-fleet/2026-08-21-backlog-grooming-regime.md) | partial — step 0 + Layers 1–2 **applied 2026-08-28** (ai-fleet PR #2350; stale-blockers + probe byte-identical, currency lint built there per PDR-010 Consequences 4 with v1.1 tunings from the first live run, groom skill, DoD lines; detail in ai-fleet's CLAUDE.md log). **Layer 3 landed 2026-09-20** (ai-fleet PR #2975) as revised v1 → v2 in place (banner at §3), carried by the 2026-09-20 prompt below. §8 extraction gate **unmet** — state measured and recorded at §8 (2026-09-20); the binding constraint is disposition, not detection. Estimation column stays contract-only per PDR-010 until telemetry coverage moves off ~15% (agent-routing-records.md, 2026-08-17). Trust model: the backlog is the substrate (conditionally — checks green + grooming on the repo owner's cadence), the roadmap is a projection of it plus product-owner statements, estimates layer on the projection |
| HopSkipInc/ai-fleet | [2026-08-28 migration numbering concurrency](ai-fleet/2026-08-28-migration-numbering-concurrency.md) | **applied 2026-08-28** — hook fix PR HopSkipInc/ai-fleet#2341 (closed #1627), numbering PR HopSkipInc/ai-fleet#2342. Two prompt defects corrected inline post-application (§2's `...HEAD` diff shape → working-tree; §3's nonexistent `--section` flag → the Consequences+Enforcement channel, Accepted Decisions being guard-locked). Found in application: `check-migration-immutability.mjs` over-fired on non-journaled `NOTES.md` (scoped to `*.sql` in #2342). Carryover: generator port + ADR-008 Decision revision → the catchup prompt below |
| HopSkipInc/analytics-infrastructure | [2026-08-28 migration numbering concurrency](analytics-infrastructure/2026-08-28-migration-numbering-concurrency.md) | **applied 2026-08-28** — PR HopSkipInc/analytics-infrastructure#603. Two prompt defects corrected inline post-application (§5's premise: this repo has **no** `generate-migration` at all; §3 missed that ADR-002 Decision §5 asserts prefix uniqueness — mediated amend refused, record predates `## Enforcement`). `ci.yml` lint job gained `fetch-depth: 0` so the token check resolves a base in CI. Carryover: ADR-002 §5 revision + the generator install/deviate decision → the catchup prompt below |
| HopSkipInc/enrichment-pipeline | [2026-08-28 migration numbering concurrency](enrichment-pipeline/2026-08-28-migration-numbering-concurrency.md) | **applied 2026-08-28** — PR HopSkipInc/enrichment-pipeline#507. Verification step 1 caught the generator's Windows-path default (`scripts\migrations` is a literal directory on Linux) — fixed in-PR; note folded into the step. No carryover |
| HopSkipInc/ai-fleet | [2026-08-28 migration numbering catchup](ai-fleet/2026-08-28-migration-numbering-catchup.md) | pending — ports the fixed `generate-migration` from enrichment-pipeline#507 (ai-fleet's copy still emits `yyyyMMddHHmmss-slug.sql`, which its own lint:adr008 check 4b now rejects) with the flat `NNNN_ISSUE_` adaptation, and revises ADR-008 Rule 4's Decision text via a new ADR amending by reference (the ADR-023→ADR-018 pattern) — the Accepted-Decision guard-lock is policy working as intended, not a blocker to route around |
| HopSkipInc/analytics-infrastructure | [2026-08-28 migration numbering catchup](analytics-infrastructure/2026-08-28-migration-numbering-catchup.md) | pending — revises ADR-002 Decision §5 via a new ADR amending by reference (its lint now contradicts the Accepted record), and forces the `generate-migration` decision on the record: the template mandates the command and this repo has none — port it (default) or record a sanctioned deviation in the Synced-templates row |
| HopSkipInc/analytics-infrastructure | [2026-09-15 append-row for singleton records](2026-09-15-append-row-singleton-records.md) | pending — **this is the repo the gap was found in.** Installed write-record is 1.1.0 (behind #91/#97/#104); registering the two new rows is what actually closes the reported problem (agents refused on `docs/testing-strategy.md` with no scripted alternative). Harness mode for these two paths stays `deny` — the mediated path is what funnels around it, no mode change needed |
| HopSkipInc/ai-fleet | [2026-09-15 append-row for singleton records](2026-09-15-append-row-singleton-records.md) | pending — installed write-record is 1.3.0, so this sync is just the #104 delta. Registering the two rows is optional here (no reported friction on these two files yet) |
| HopSkipInc/enrichment-pipeline | [2026-09-15 append-row for singleton records](2026-09-15-append-row-singleton-records.md) | pending — installed write-record is 1.2.0, catches up on #97 (section-matching hardening — read the ai-fleet 2026-08-19 prompt's install-rule section first) and #104 together. Registering the two rows is optional here |
| HopSkipInc/ai-fleet | [2026-09-16 audit domains pointer](ai-fleet/2026-09-16-audit-domains-pointer.md) | pending — **carries three migrations**, one per audit machine, replacing the inline domain block in each stored goal with a pointer to `docs/audit-domains.md`. Not a file copy: the domain list lives in Postgres here, and `.github/workflows/scheduled-audit.yml` is not installed and should not be. Converges machines that have already drifted (`0258` added code-hygiene to several, `0375` added quality+coverage to `audit-enrichment-pipeline` only), so the first audit after it lands runs domains its machine was not running — a behaviour change the migration notes must state. Adds domain 9 (configuration and secrets), which reports SKIPPED here until the configuration-governance records file exists. Open question returning upstream: `0291` sweeps `docs/competitive-intel/` while upstream domain 5 globs `docs/watch-items/` |
| HopSkipInc/ai-fleet | merge-path 1.0.0 + 1.1.0 (no downstream prompt — applied direct from upstream #109/#110) | applied 2026-09-19 (ai-fleet PR #2929) — `docs/merge-path.md` 1.1.0, `scripts/check-merge-path.mjs` 1.1.0, `.github/workflows/merge-path-probe.yml` 1.0.0 (the probe had been a local undeclared 1.0.0 copy; replaced by the upstream template). All three declared in the Synced-templates table; re-verified byte-identical to the merged templates 2026-09-20 |
| HopSkipInc/ai-fleet | [2026-09-20 roadmap layer 3 v2](ai-fleet/2026-09-20-roadmap-layer3-v2.md) | applied 2026-09-20 (ai-fleet PR #2975) — full v2 scope: shared graph loader extracted output-pure (currency-lint fixture byte-identical pre/post), `generate-roadmap.mjs` + first committed `docs/roadmap-data.md` (generated trust header, no wall-clock; renders **UNTRUSTED** by design on cold start — no groom marker ever posted), six-column roadmap table (derived status only, full-ref provenance), age-bucketed debt table, backlog-groom 1.1.0 closing marker, `roadmap-probe.yml` (weekly + dispatch, human merges), narrative + debt-sweep skills, fixture suite wired into `run-tests.yml`. First live census: 391 unparented (368 actionable + 23 deferred) of 526 open vs the 412/525 baseline — within drift; definitional deltas (census counted "no epic", the table also excludes parents and linked children; >90d excludes deferred) named in the PR report-back. Report-backs: stale-blockers loader unification is owed **upstream** as a template revision; uncomputable columns named in the script header (`expires`, `credential` block type, PR state — no deterministic epic↔PR join). Extraction gate for a `templates/` artifact stays open on the prompt's five conditions; Layers 1–2's §8 gate is separate and unmet. Post-merge follow-ups pending: probe `workflow_dispatch`, first narrative pass, first sweep dry run |
| HopSkipInc/ai-fleet | [2026-09-24 deepseek all tiers](ai-fleet/2026-09-24-deepseek-all-tiers.md) | pending — owner-directed routing calibration **carrying a migration**: `deepseek-v4-1-flash` added to the `deep-reasoning` class in `docs/agent-routing-class-map.json` (`live_binding` → `deepseek-v4-1-flash@deepseek`, regenerated `.md`), plus a replace-set migration (append-only rebind + grandfathered eligibility via 0513 + re-ranked grants, deepseek rank 1 / `gpt-6-sol@azure` rank 2 fallback) so a fleet actually **resolves** deep-reasoning to deepseek; policy re-sync 1.15.0 → 1.16.0, records §1 note + CLAUDE.md step-2 clarification. Recorded ADR-066 decision-5 violation (evidence owed #3048), same branch as 0517/0518/0519. Moves `code-critic`/`investigator` onto deepseek for the run |
| HopSkipInc/analytics-infrastructure | [2026-09-24 deepseek all tiers](2026-09-24-deepseek-all-tiers.md) | pending — records §1 `DeepSeek V4.1 Flash` → `frontier` (+ route, note, review-log) and the CLAUDE.md step-2 clarification; the `docs/agent-routing.md` re-sync to 1.16.0 rides the pending routing re-sync row |
| HopSkipInc/enrichment-pipeline | [2026-09-24 deepseek all tiers](2026-09-24-deepseek-all-tiers.md) | pending — records §1 `frontier` + route + note and the CLAUDE.md step-2 clarification; the policy re-sync rides the pending routing re-sync row |
| HopSkipInc/ai-fleet | [2026-09-26 epic handoff](ai-fleet/2026-09-26-epic-handoff.md) | pending — `docs/epic-handoff.md` + `scripts/check-epic-handoff.mjs` + `workflows/epic-handoff-probe.yml` all v1.0.0; DoD 1.6.0 and issue-authoring 1.4.0 deltas **hand-ported** (both files are Adapted here). **Overlap boundary named in the prompt:** the probe reads epic bodies only — `check-backlog-currency.mjs` keeps the body-vs-graph TRIGGER (one axis, one enumerator, PDR-008). Probe, never a gate; `missing` report-only on first adoption |
| HopSkipInc/analytics-infrastructure | [2026-09-26 epic handoff](analytics-infrastructure/2026-09-26-epic-handoff.md) | pending — **source repo of the practice** (epic #229's live section, applied 2026-09-25). Install the three artifacts; hand-port the DoD/issue-authoring deltas into the Adapted files; canonicalize #229's date from the legacy heading form to the `**Handoff updated:**` line. No grooming regime here, so this probe is the only epic-body freshness backstop |
| HopSkipInc/enrichment-pipeline | [2026-09-26 epic handoff](enrichment-pipeline/2026-09-26-epic-handoff.md) | pending — install the three artifacts (verify the synced-lint directory — `scripts/` vs `tools/`); hand-port the DoD/issue-authoring deltas into the Adapted files. No grooming regime and no stale-blocker probe here, so this is the only epic-body freshness backstop |

**Epic-handoff set is three repos, not four.** `infra-ops` is excluded deliberately: core
class and solo-maintained, with no agent-worked epic backlog — there is no handoff for a
session to pick up, and the probe would have no `epic`-labelled issues to read. Recorded so
the omission is considered, not silent. (The convention itself is class-agnostic; if an
epic backlog appears there, install the three artifacts and ignore this note.)

**DeepSeek-all-tiers set is three repos, not four.** `infra-ops` is excluded deliberately: the
routing layer was never adopted there (core class, PDR-009) — no `docs/agent-routing-records.md`
and no `## Agent routing` section in its CLAUDE.md, so there is nothing to reclass and no gate
to unblock. Recorded so the omission is considered, not silent.
